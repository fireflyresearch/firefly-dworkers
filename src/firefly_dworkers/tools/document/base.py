...
class DocumentTool(BaseTool):
    ...
    async def create(self, *, title: str = "", sections: list[SectionSpec] | None = None) -> bytes:
        """Create a document and return the raw file bytes."""
        return await self._create_document(title, sections or [])

    async def create_and_save(
        self, output_path: str, *, title: str = "", sections: list[SectionSpec] | None = None
    ) -> str:
        """Create a document and save it to *output_path*. Returns the absolute path."""
        data = await self.create(title=title, sections=sections)
        with open(output_path, "wb") as f:
            f.write(data)
        return os.path.abspath(output_path)

    async def modify(self, source: str, *, operations: list[DocumentOperation] | None = None) -> bytes:
        """Modify a document and return the raw file bytes."""
        return await self._modify_document(source, operations or [])

    async def modify_and_save(
        self, source: str, output_path: str, *, operations: list[DocumentOperation] | None = None
    ) -> str:
        """Modify a document and save it to *output_path*. Returns the absolute path."""
